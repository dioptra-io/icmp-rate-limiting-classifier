...
def traceroute(destination, disrupt_dir, is_simultaneous_midar):
    traceroute_cmd = "traceroute -n " + destination + " > " + disrupt_dir+destination+".traceroute"
...
def midar(candidate, disrupt_dir):
    ...
    midar_cmd = "cd resources/results/survey/midar/disrupt; " \
...
def test_midar_disrupt(candidate, disrupt_dir):
    ...
    traceroute_thread = Thread(target=traceroute, args=(candidate,
                                                        disrupt_dir, True, ))
