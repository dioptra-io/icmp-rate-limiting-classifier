...
def simultaneous_phase_impl(
    ...
    print(
        "Number of candidates after hop based optimization: "
        + str(len(close_candidates))
    )
    ...
    print(
        "Writing the candidates for the cpp tool in: "
    ...
    if is_group_already_probed(high_rate_candidate, cpp_options.pcap_dir_groups):
        print(
            "Group already present in individual files, skipping candidates rate limiting algorithm..."
        )
        only_analyse = True
    else:
        print(
            "Executing rate limiting algorithm for candidates... ", end="", flush=True
        )
        only_analyse = False

    cpp_output_file = (
        cpp_options.output_file
        + "_cluster"
        + str(cluster_triggering_rate)
        + "_"
        + str(iteration)
...
def merge_results_file(icmp_install_dir, node, results_dir, columns):
    ...
    df_individual.to_csv(
        icmp_install_dir + "resources/results/survey_individual6_paper", index=False
    )
...
if __name__ == "__main__":
    ...
    if False:
        ...
        for filename in os.listdir(results_dir):
            ...
            if stable_iter == "0":
                cpp_output_file = (
                    "/root/ICMPRateLimiting/test_cluster2048_"
                    + iter
                    + "_"
                    + stable_iter
                )
                ...
                next_alias_file = (
                    alias_prefix + "_" + iter + "_" + str(next_stable_iter)
                )
                if os.path.exists(next_alias_file):
                    ...
                    previous_alias_file = (
                        alias_prefix + "_" + iter + "_" + str(previous_stable_iter)
                    )
                    previous_previous_alias_file = (
                        alias_prefix
                        + "_"
                        + iter
                        + "_"
                        + str(previous_previous_stable_iter)
                    )
                    if os.path.isfile(previous_previous_alias_file) and os.path.isfile(
                        previous_alias_file
                    ):

                        if (
                            os.stat(previous_alias_file).st_size
                            == os.stat(previous_previous_alias_file).st_size
                        ):
