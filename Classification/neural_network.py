...
def create_training_input_fn(
    ...
    def _input_fn(num_epochs=None, shuffle=True):
        ...
        raw_features = {
            feature_column: feature_values.reindex(idx)
            for feature_column, feature_values in features.iteritems()
        }
...
def train_nn_classification_model(
    ...
    for period in range(0, periods):
        ...
        if is_multilabel:
            training_pred_labels = np.round(
                [item["probabilities"] for item in training_predictions]
            )
        else:
            training_pred_labels = np.array(
        ...
        if is_multilabel:
            validation_pred_labels = np.round(
                [item["probabilities"] for item in validation_predictions]
            )
        else:
            validation_pred_labels = np.array(
        ...
        precision, recall, accuracy, f_score = compute_metrics(
            training_pred_labels, training_targets
        )
        ...
        precision, recall, accuracy, f_score = compute_metrics(
            validation_pred_labels, validation_targets
        )
        print("Precision: " + str(precision))
        print("Recall: " + str(recall))
        print("Accuracy: " + str(accuracy))
    ...
    _ = map(
        os.remove, glob.glob(os.path.join(classifier.model_dir, "events.out.tfevents*"))
    )
    ...
    if is_multilabel:
        final_predictions = np.round(
            [item["probabilities"] for item in final_predictions]
        )
    else:
    ...
    precision, recall, accuracy, f_score = compute_metrics(
        final_predictions, validation_targets
    )
