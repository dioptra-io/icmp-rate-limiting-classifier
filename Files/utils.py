...
def build_candidate_line(
    group, af_family, probing_type, probing_protocol, real_target, probing_target
):
    return (
        group
        + ", "
    ...
    with open(ip_file) as ip_file_fp:
        with open(output_file, "w") as output_file_fp:
            for line in ip_file_fp:
                ip = line.strip("\n")
                candidate_line = build_candidate_line(
                    group, af_family, probing_type, probing_protocol, ip, ip
                )
